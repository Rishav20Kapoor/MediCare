...
export default function EditProfilePage({ apiBase }) {
  ...
  const API_BASE = `${(import.meta.env.VITE_API_URL || "http://localhost:4000").replace(/\/$/, "")}/api/doctors`;
  ...
  useEffect(() => {
    ...
    async function fetchDoctor() {
      try {
        ...
        // normalize patients to a numeric value or empty string so number inputs accept it
        if (d.patients !== undefined && d.patients !== null) {
          if (typeof d.patients === "number") {
            // keep as-is
          } else {
            const m = String(d.patients).match(/(\d+)/);
            d.patients = m ? Number(m[1]) : "";
          }
        }
        d.imageUrl =
  ...
  // to reset the profile bakc to initial
