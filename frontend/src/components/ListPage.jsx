...
const ListPage = () => {
    ...
    // update the status it is similar to dashboard page
    ...
    <div className = {ls.pageContainer}>
        <div className = {ls.contentWrapper}>
            <div className = {ls.headerContainer}>
                <div>
                    <h1 className = {ls.headerTitle}> All Appointments </h1>
                    <p className = {ls.headerSubtitle}> Latest at top - search by patient name </p>
                </div>
                <div className = {ls.searchFilterContainer}>
                    <div className = {ls.searchContainer}>
                        <div className = {ls.searchIconContainer}>
                            <Search className = {ls.searchIcon}/>
                        </div>
                        <input 
                            value={search} 
                        ...
                        />
                        {/* if we search something */}
                        {search && (
                            <button onClick={() => setSearch("")} className = {ls.clearSearchButton}>
                                <X className = {ls.clearSearchButton}/>
                            </button>
                        )}
                    </div>
                    {/* status filter */}
            ...
            {loading ? (
                <div className = {ls.loadingContainer}>
                    Loading Appointments...
                </div>
            ) : error ? (
                <div className = {ls.errorContainer}> Error: {error}</div>
            ) : (
                <div className = {ls.appointmentsGrid}>
                    {filtered.map((a) => (
                        <article key = {a.id} className = {ls.appointmentCard}>
                            <header className = {ls.cardHeader}>
                                <div className = {ls.cardAvatar}>

                                    {a.doctorImage ? (
                                        <img src={a.doctorImage} alt = {a.doctorName}
                                            className = {ls.cardAvatarImage}
                                            onError={(e) =>
                                            (e.currentTarget.style.display = "none")
                                            }
                                        />
                                    ) :(
                                        <div className = {ls.cardAvatarFallback}>
                                ...
                                <div className = {ls.cardContent}>
                                    <div className = {ls.cardPatientName}>
                                        {a.patient}
                                    </div>
                                    <div className = {ls.cardPatientInfo}>
                                        {a.age}yrs $middot; {a.gender}
                                    </div>
                                    <div className = {ls.cardDoctorInfo}>
                                        <span className = {ls.cardDoctorName}>
                                            {a.doctorName}
                                        </span>
                                    </div>
                                    <div className = {ls.cardSpeciality}>
                                        {a.speciality}
                                    </div>
                                </div>
                                ...
                                <div className = {ls.contactStatusSection}>
                                    <div className = {ls.phoneContainer}>
                                        <Phone className = {ls.phoneIcon} />
                                        <span className = {ls.phoneNumber}>
                                            {a.mobile}
                                        </span>
                                    </div>
